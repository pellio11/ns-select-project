...
class Tribe__Events__Tickets__Woo__Main extends Tribe__Events__Tickets__Tickets {
		...
		foreach ( (array) $order_items as $item ) {
			...
			if ( ! empty( $event_id ) ) {
				...
				for ( $i = 0; $i < $quantity; $i ++ ) {
					...
					do_action( 'wootickets_generate_ticket_attendee', $attendee_id, $event_id, $order );
				}
			}
		}
	...
	public function save_ticket( $event_id, $ticket, $raw_data = array() ) {
		...
		do_action( 'wootickets_after_' . $save_type . '_ticket', $ticket->ID );
		do_action( 'wootickets_after_save_ticket', $ticket->ID );
